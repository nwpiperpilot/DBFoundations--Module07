...
-- 2022-02-28,DSarver,Assignment07_DSarver.sql
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_DSarver')
	 Begin 
	  Alter Database [Assignment07DB_DSarver] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_DSarver;
	 End
	Create Database Assignment07DB_DSarver;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_DSarver;
...
SELECT c.CategoryName, p.ProductName, FORMAT(p.UnitPrice, 'C', 'en-us')
...
AS
SELECT TOP 100000 p.ProductName, FORMAT(i.InventoryDate,'MMMM, yyyy') AS InventoryDate, i.Count 
	FROM vProducts AS p 
...
AS
SELECT TOP 100000 CategoryName AS [Category Name], FORMAT(i.InventoryDate,'MMMM, yyyy') AS [Inventory Date], SUM(i.Count) AS [Inventory Count]
    FROM vProducts AS p
        JOIN vInventories AS i
        ON p.ProductID = i.ProductID
